...
当前 PR 暴露 4 个 memory consistency IR。它们都没有 SSA operand 和 result；参数通过
PTO attr 表达。PyPTO 可以直接传 enum，binding 会自动构造对应 attr。

| IR | 参数 | 当前支持取值 | MLIR 写法 | PyPTO 写法 |
| --- | --- | --- | --- | --- |
| `pto.fence.release` | `scope: FenceScopeAttr` | `ddr` | `pto.fence.release #pto.fence_scope<ddr>` | `pto.FenceReleaseOp(pto.FenceScope.DDR)` |
| `pto.fence.acquire` | `scope: FenceScopeAttr` | `ddr` | `pto.fence.acquire #pto.fence_scope<ddr>` | `pto.FenceAcquireOp(pto.FenceScope.DDR)` |
...
| `pto.cmo.invalidate` | `space: AddressSpaceAttr`，粒度固定为 `all` | `gm` | `pto.cmo.invalidate all #pto.address_space<gm>` | `pto.CmoInvalidateOp(pto.AddressSpace.GM)` |
