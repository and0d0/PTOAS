...
static FailureOr<StringRef> buildL1CacheLoadCallee(MLIRContext *context,
  ...
  if (auto intType = dyn_cast<IntegerType>(resultType)) {
    ...
    elem = "s64";
  } else if (auto payloadBits =
                 pto::getPTOPackedFloat8VectorPayloadBitWidth(resultType)) {
    if (*payloadBits == 16)
      elem = "s16";
    else if (*payloadBits == 32)
      elem = "s32";
    else if (*payloadBits == 64)
      elem = "s64";
...
static FailureOr<StringRef> buildL1CacheStoreCallee(MLIRContext *context,
  ...
  if (auto intType = dyn_cast<IntegerType>(valueType)) {
    ...
    elem = "b64";
                 ...
                 pto::getPTOPackedFloat8VectorPayloadBitWidth(valueType)) {
    if (*payloadBits == 16)
      elem = "b16";
    else if (*payloadBits == 32)
      elem = "b32";
...
static Value convertLdgCallResult(Location loc, Type valueType,
  ...
  if (auto payloadBits =
          pto::getPTOPackedFloat8VectorPayloadBitWidth(valueType)) {
    Type payloadType = *payloadBits == 64 ? rewriter.getI64Type()
                       : *payloadBits == 32 ? rewriter.getI32Type()
                                            : rewriter.getI16Type();
    Value payload = callResult;
    if (*payloadBits == 16)
      payload =
          rewriter.create<arith::TruncIOp>(loc, payloadType, callResult);
    return rewriter.create<LLVM::BitcastOp>(loc, convertedValueType, payload);
  }
  if (pto::isPTOFloat8Type(valueType) || pto::isPTOHiFloat8Type(valueType)) {
    Value payload =
...
static Value convertStgValue(Location loc, Type valueType, Value value,
  ...
  if (auto payloadBits =
    ...
    Type payloadType = *payloadBits == 64 ? rewriter.getI64Type()
                       : *payloadBits == 32 ? rewriter.getI32Type()
                                            : rewriter.getI16Type();
    Value payload = rewriter.create<LLVM::BitcastOp>(loc, payloadType, value);
    if (*payloadBits != 16)
      return payload;
