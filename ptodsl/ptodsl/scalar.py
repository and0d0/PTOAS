...
def load(ptr_or_ref, offset=None, *, contiguous=None):
    ...
    if _is_local_allocated_buffer(allocated_buffer):
        return wrap_surface_value(_emit_llvm_load(buffer_value, index_value, result_type))
...
def store(value, ptr_or_ref, offset=None, *, contiguous=None):
    ...
    if _is_local_allocated_buffer(allocated_buffer):
        _emit_llvm_store(
            coerce_scalar_to_type(value, elem_type, context="scalar.store(...)"),
            buffer_value,
            index_value,
            elem_type,
        )
...
def _emit_llvm_load(buffer_value, index_value, elem_type):
    ptr_value = _emit_llvm_byte_pointer(buffer_value, index_value, elem_type)
    return llvm.LoadOp(elem_type, ptr_value).res
...
def _emit_llvm_store(value, buffer_value, index_value, elem_type):
    ptr_value = _emit_llvm_byte_pointer(buffer_value, index_value, elem_type)
    llvm.StoreOp(value, ptr_value)
